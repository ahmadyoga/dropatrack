// Content script — runs on regular YouTube pages (www.youtube.com)
...
(() => {
  ...
  // ─── Inject banner into playlist panel ────────────────────────────
  ...
  // ─── Inject per-item buttons into playlist ────────────────────────
  ...
  function buildBannerHTML() {
    ...
    if (hasRoom) {
      html += `<button class="dropatrack-btn-add-all" id="dropatrack-btn-add-all" title="Add playlist to room">+ All</button>`;
    }
    ...
    if (addAllBtn) {
      addAllBtn.addEventListener('click', async () => {
        ...
        if (!info.playlistVideos.length) {
          ...
          setTimeout(() => { addAllBtn.textContent = '+ All'; }, 2000);
          return;
        }
